...
echo "No deployment instructions yet"
DEPLOYMENT
##### ----------------------------------------------- #####
...
$fishy_config = <<-'FISHY'
#!/bin/bash
#Syntax: ./script.bash [control|domain1|domain2]

echo "Deploying FISHY sandbox Kubernetes cluster..."
controls=$(cat /vagrant/control.ip)
domain1=$(cat /vagrant/domain1.ip)
domain2=$(cat /vagrant/domain2.ip)
domain_config=""
...
Vagrant.configure("2") do |config|
  config.vm.define "control", primary: true do |control|
    ...
    control.vm.provision :shell,
      inline: "echo `ip addr sh eth0 | grep 'inet ' | awk '{print substr($2, 1, length($2)-3)}'` > /vagrant/control.ip"
  #disable all the shell beeps
    control.vm.provision :shell,
      ...
      control.vm.provision :shell,
        inline: "sleep 10"
      control.vm.provision :shell,
        inline: $fishy_config, args: "control"
      control.vm.provision :shell,
        inline: "sleep 20 && rm /vagrant/*.ip"
  ...
  (1..$NODE_COUNT).each do |i|
    config.vm.define "domain#{i}" do |domain|
      ...
      domain.vm.network "private_network", type: "dhcp"

      domain.vm.provision :shell,
        inline: "echo `ip addr sh eth0 | grep 'inet ' | awk '{print substr($2, 1, length($2)-3)}'` > /vagrant/domain#{i}.ip"
      domain.vm.provision :shell,
        inline: "sleep 10"
      domain.vm.provision :shell,
        inline: "echo 'set bell-style none' >> /etc/inputrc \
      ...
      domain.vm.provision :shell,
        inline: $fishy_config, args: "domain#{i}"
    end
  end
end
